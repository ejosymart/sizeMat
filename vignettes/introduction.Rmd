---
title: 'sexMat: An R Package to Estimate Size at Sexual Maturity'
...
This vignette introduces you to `sexMat` package and provide a way to estimate **morphometric and gonadal size at sexual maturity**.
...
For estimating morphometric maturity use the *crabdata* base. Ths data set containing allometric measures and other atribbutes (year, month, sex category) of 223 crabs of the specie *Chionoectes tanneri*.
...
The `classify_mature` function requires a data.frame (e.g. crabdata) with alometric variables and sex category. The argument `varNames` requires the name of two allometric variables only, and `varSex` requires the name of the variable containing sex information. If the argument `selecSex` is `NULL` all the individuals will be used in the classification analysis. Finally the `method` is focus in the discriminant analysis to be used ("ld": linear discriminant analysis, "qd": quadratic discriminant analysis). We reccomend begin the analysis with the `method = "ld"`.
...
The `morph_mature` function use the logit approach (frequentist or bayesian). The morphometric size at 50% maturity ($L_{50}$) was estimated as the length at which a randomly chosen specimen has a 50% chance of being mature (Somerton  1980, Roa  et al.  1999). 

In the regression analysis, $X$ (e.g: carapace width) is considered the explanatory variable and the classification of maturity $CS$ (juvelines: 0, adults: 1) is considered the response variable (binomial). The variables are fitted to a logit function with the form: 
...
The `morph_mature` function requires an object of class "classify" with the X, Y (allometric variables) and classification of maturity (juvelines = 0, adults = 1).
...
## Plot maturity ogive
For plotting the maturity ogive, an object of class "morphMat"is required. The function `plot` generates 4 graphics: 1), 2) and 3) are histograms for the A, B parameters and the morphometric size at sexual maturity (L50), the last is the maturity ogive.
...
* `stage_mat`: I, II, III, IV, where I is considered inmature.
...
The `gonad_mature` function use the logistic approach (see section: Estimate morphometric size at sexual maturity).

The `gonad_mature` function requires a data.frame with allometric variables (e.g: total length, fork length) and a variable containing the stages of sexual maturity. The function will use one of the allometric variables only, and it is specipied in the `varNames` argument. 

The argument `varNames` requires a character string indicating the name of the allometric and the stage of sexual maturity variable to be used for analysis (e.g `varNames = c("total_length", "stage_mat")`).

The arguments `inmName` and `matName` require a character string indicanting the name of the stages of sexual maturity in the data.frame. The arguments could be a vector (e.g `matName = c("II", "III", "IV")`). The variable `stage_mat` in the *matFish* database, contains the stages of the sexual maturity. In this case, stage I is considered inmature and II, III, IV are considered mature. Then the stages of sexual maturity are transformed in a binomial variable where inmature = 0 and mature = 1.
...
The output is an object of class "gonadMat". This object contains a dataframe with the allometric variable "X" and stage of sexual maturity (inmature = 0, mature  = 1). Also the fitted values for the curve logistic regression and confidence intervals (95\%). If you print the object, the median of the gonadal size at sexual maturity estimation (L50) and parameters are shown.

## Estimate gonadal size at sexual maturity (maturity ogive estimation)
```{r, echo=TRUE}
#Frequentist regression 
...
## Plot maturity ogive
For plotting the maturity ogive the object of class "gonadMat"is required. The function `plot` generates 4 graphics: 1), 2) and 3) are histograms for the A, B parameters and the morphometric size at sexual maturity (L50), the last is the maturity ogive.
