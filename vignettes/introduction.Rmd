---
title: "Extending SexMat R-package"
date: "`r Sys.Date()`"
output: 
  rmarkdown::html_vignette:
    fig_caption: yes
vignette: >
  %\VignetteIndexEntry{Extending SexMat R-package}
...
This vignette introduces you to SexMat functions and shows you how to apply them to estimate morphometric and gonadal size at sexual maturity.
...
Print results of the "classify_data" object. Shows the number of juveniles and adults after classification and the regression analysis for juveniles and adults. 
```{r, echo = TRUE}
print(classify_data)
```
...
The `morph_mature` function use the logistic approach (frequentist or bayesian). The morphometric size at 50% maturity ($L_{50}$) was estimated as the length at which a randomly chosen specimen has a 50% chance of being mature (Somerton  1980, Roa  et al.  1999). 

In the regression analysis, $X$ (e.g: carapace width) is considered the explanatory variable and the classification of maturity $CS$ (juvelines: 0, adults: 1) is considered the response variable (binomial). The variables are fitted to a logistic function with the form: 
...
## Plot maturity ogive
For plotting the maturity ogive the object of class "morphMat"is required. The function `plot` generates 4 graphics: 1), 2) and 3) are histograms for the A, B parameters and the morphometric size at sexual maturity (L50), the last is the maturity ogive.
...
For estimating gonadal maturity use the *matFish* database. This data base contains two variables: 
* `length`: total length in cm.
* `stage`: I, II, III, IV, where I is considered inmature and the others mature.
...
The `gonad_mature` as the `morph_mature` function use the logistic approach (see section: Estimate morphometric size at sexual maturity).

The `gonad_mature` function requires a data.frame with the X (allometric variable, e.g: total length, fork length) and stages of sexual maturity. 

The arguments `inmName` and `matName` require a character string indicanting the name of the stages of sexual maturity in the data. The arguments could be a vector (e.g `matName = c("II", "III", "IV")`). The variable `stage` in the *matFish* database, contains the stages of the sexual maturity. In this case, stage I is considered inmature and II, III, IV are considered mature. Then the this stages of sexual maturity are transformed in a binomial variable where inmature = 0 and mature = 1.
...
```{r, echo=TRUE}
#Frequentist regression 
my_ogive_fq = gonad_mature(matFish, inmName = "I", matName = c("II", "III", "IV" ), 
                           method = "fq", niter = 999)
...
#Bayesian regression 
my_ogive_bayes = gonad_mature(matFish, inmName = "I", matName = c("II", "III", "IV" ), 
