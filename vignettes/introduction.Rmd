...
## Estimate size at sexual maturity (maturity ogive estimation)
The `calculate_mature` use the logistic approach (frequentist or bayesian). The size at 50% maturity ($L_{50}$) was estimated as the length at which a randomly chosen specimen has a 50% chance of being mature (Somerton  1980, Roa  et al.  1999). 
...
## Plot maturity ogive
For plotting the maturity ogive you need the object of class "mature". But the function `plot` generates 4 graphics: 1), 2) and 3) are histograms for the A, B parameters and the size at sexual maturity (L50), the last is the maturity ogive.
